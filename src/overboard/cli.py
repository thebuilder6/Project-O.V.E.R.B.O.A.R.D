...
from .core.robot_model import RobotConfig
from .core.factory import Optimizer
from .validation.validator import validate_trajectory
from .visualization.plotter import plot_trajectory, plot_reachability_envelope
from .visualization.animated_plotter import animate_trajectory
from .visualization.convergence_plotter import plot_convergence
from .visualization.live_visualizer import get_visualizer
from .utils.export import write_controller_file, write_python_file

# JAX/Immrax Imports
try:
    from .validation.immrax_validator import ImmraxValidator
    from .core.jax_robot_model import JAXRobotConfig
    HAS_JAX = True
except ImportError:
    HAS_JAX = False
...
def main(config, waypoints, output, samples, accuracy_weight, stop_waypoints, events,
    ...
    method = "simple" if simple else "multiverse"
    optimizer = Optimizer(
        config_data,
        method=method,
    ...
    for attempt in range(max_retries):
        ...
        if HAS_JAX and (validate or live):
            ...
            # Attach envelope to samples for export/viz
    ...
    if benchmark:
        from .validation.validator import compute_metrics
    ...
    if show_convergence:
        if hasattr(optimizer._optimizer, 'iteration_history') and optimizer._optimizer.iteration_history:
            plot_convergence(optimizer._optimizer.iteration_history, mode=convergence_mode, waypoints=wps, title=f"Convergence: {os.path.basename(output)}")
